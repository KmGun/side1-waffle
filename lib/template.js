const mastercalRT =
"1//0ej6a_pa17-DWCgYIARAAGA4SNwF-L9IretvchlZCX0lryJOqwyXHoXqaLELcVb5GHq344i5bTADMqAVNiJn1krkJB4a6JYmxblQ";
...
exports.getMasterCalData = async function (req, res) {
  masterCal_list = await fetch(
    `https://www.googleapis.com/calendar/v3/users/me/calendarList?access_token=${req.user.mastertoken.access_token}`
  )
...
exports.makeMasterCalDB = function (masterCal_list) {
  ...
  while (i < masterCal_list.items.length) {
    ...
    db.query(
      "SELECT * FROM masterCal WHERE calId = ?",
      [calId],
      function (err, cal) {
        if (!cal[0]) {
          db.query(
            "INSERT INTO masterCal (calId,calName) VALUES(?,?)",
            [calId, calName],
            function (err, result) {}
          );
        }
...
exports.getMasterCalEventLists = async function (req, res, calId, data) {
  await fetch(
    `https://www.googleapis.com/calendar/v3/calendars/${calId}/events?access_token=${req.user.mastertoken.access_token}`
  )
    .then(function (response) {
      return response.json();
    })
...
// 테스트 필요
exports.createMasterCal = async function (req, res, tempData) {
  const postObj = {
    summary: req.body.summary,
  };

  await fetch(
    `https://www.googleapis.com/calendar/v3/calendars?access_token=${req.user.mastertoken.access_token}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
// exports.addMasterCalHashtagsOnDB = function (req, res, tempData) {
//   const hashtags = JSON.stringify(req.body.hashtags);
//   db.query(
//     "INSERT INTO masterCal (calId,calName,hastags) VALUES(?,?,?)",
//     [tempData.calId, tempData.calName, hashtags],
//     function (err, result) {}
//   );
// };
...
exports.subscribeCal = async function (req, res, calId) {
  await fetch(
    `https://www.googleapis.com/calendar/v3/users/me/calendarList?access_token=${req.user.token.access_token}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
exports.addSubscriberOnDB = function (req, res, subscribers) {
  ...
  db.query(
    `UPDATE masterCal SET subscribers=? WHERE id=?;`,
    [subs, req.params.Id],
    function (err, result) {
...
exports.unsubscribeCal = async function (req, res, calId) {
  await fetch(
    `https://www.googleapis.com/calendar/v3/users/me/calendarList/${calId}?access_token=${req.user.token.access_token}`,
    {
      method: "DELETE",
    }
  );
